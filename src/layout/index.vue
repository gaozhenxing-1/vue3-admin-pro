<template>
  <div style="display:flex;height:100vh" :class="isDark ? 'dark' : ''">
    ...
    <div style="flex:1;display:flex;flex-direction:column;overflow:hidden">
      <div style="height:50px;background:var(--navbar-bg,#fff);display:flex;align-items:center;justify-content:space-between;padding:0 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);flex-shrink:0">
        ...
        <div>
          <el-dropdown trigger="click" :hide-on-click="false">
            <span style="display:flex;align-items:center;gap:6px;cursor:pointer;color:var(--text-regular,#606266);font-size:14px">
            ...
            <template #dropdown>
              <el-dropdown-menu style="width:260px;padding:8px 0">
                ...
                <div style="padding:8px 16px 4px">
                  <div style="font-size:12px;color:var(--text-secondary,#909399);margin-bottom:8px">
                ...
                <el-dropdown-item @click="toggleDark">
                  <div style="display:flex;align-items:center;justify-content:space-between;width:100%">
                    <span>{{ isDark ? (lang === 'zh-cn' ? '浅色模式' : 'Light Mode') : (lang === 'zh-cn' ? '暗色模式' : 'Dark Mode') }}</span>
                ...
                <el-dropdown-item @click="toggleLang">
                  <div style="display:flex;align-items:center;justify-content:space-between;width:100%">
                    ...
                    <span style="font-size:12px;font-weight:700;color:var(--el-color-primary)">{{ lang === 'zh-cn' ? 'EN' : '中' }}</span>
                  </div>
                </el-dropdown-item>
...
import {
  Fold, Expand, ArrowDown, UserFilled, Sunny, Moon, SwitchButton,
...
const iconMap: Record<string, any> = { Odometer, DataAnalysis, Grid, Document, User, Avatar, Tickets, Folder, Bell, Setting, EditPen }
...
const breadcrumbs = computed(() => {
  return route.matched.filter((r) => r.meta?.title && !r.meta?.hidden).map((r) => ({ path: r.path, title: r.meta?.title as string }))
})

function handleSelect(index: string) { router.push(index) }
function switchTheme(primary: string) { currentTheme.value = primary; applyTheme(primary) }
function switchStyle(mode: 'tech' | 'minimal') { currentStyle.value = mode; applyStyle(mode) }
function toggleDark() { isDark.value = !isDark.value; document.documentElement.classList.toggle('dark', isDark.value); localStorage.setItem('darkMode', isDark.value ? 'true' : 'false') }
function toggleLang() { const next = lang.value === 'zh-cn' ? 'en' : 'zh-cn'; lang.value = next; currentLocale.value = next; localStorage.setItem('locale', next) }
...
function removeTag(path: string) {
  ...
  if (path === route.path && visitedTags.value.length > 0) router.push(visitedTags.value[visitedTags.value.length - 1].path)
}

watch(() => route.path, () => {
  if (route.meta?.hidden) return
  const exists = visitedTags.value.find(t => t.path === route.path)
  if (!exists) visitedTags.value.push({ path: route.path, title: (route.meta?.title as string) || route.path })
  if (route.meta?.keepAlive && !cachedNames.value.includes(route.name as string)) cachedNames.value.push(route.name as string)
}, { immediate: true })
</script>
