  ...
  <div class="login-container">
    <div class="login-card">
      ...
      <el-form ref="formRef" :model="form" :rules="rules" class="login-form">
        ...
        <el-form-item>
          <el-button type="primary" size="large" class="login-btn" @click="handleSubmit">{{ t('sendResetLink') }}</el-button>
        </el-form-item>
      </el-form>
      <div class="login-tips"><span><router-link to="/login">{{ t('backToLogin') }}</router-link></span></div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { reactive, ref } from 'vue'; import { Message } from '@element-plus/icons-vue'
...
const formRef = ref<FormInstance>(); const form = reactive({ email: '' })
const rules: FormRules = {
  email: [{ required: true, message: '请输入邮箱', trigger: 'blur' }, { type: 'email', message: '邮箱格式不正确', trigger: 'blur' }],
}
function handleSubmit() { formRef.value?.validate((v) => { if (v) ElMessage.success(t('resetLinkSent')) }) }
</script>
