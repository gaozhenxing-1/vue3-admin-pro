  ...
  <div class="user-page">
    <el-card shadow="hover">
      <div class="search-bar">
        <el-input v-model="query.keyword" :placeholder="t('searchUsername')" clearable style="width: 240px" @clear="fetchData" @keyup.enter="fetchData" />
        <el-select v-model="query.status" :placeholder="t('allStatus')" clearable style="width: 130px; margin-left: 12px" @change="fetchData">
      ...
      <div class="pagination-wrapper">
        <el-pagination
          ...
          layout="total, sizes, prev, pager, next, jumper" background @change="fetchData"
        />
      </div>
    </el-card>

    <el-dialog v-model="dialogVisible" :title="isEdit ? t('editUser') : t('addUser')" width="520px" destroy-on-close>
...
interface User { id?: number; username: string; nickname: string; email: string; role: string; status: boolean; createdAt?: string }

const loading = ref(false); const dialogVisible = ref(false); const isEdit = ref(false)
const formRef = ref<FormInstance>()
const tableData = ref<User[]>([]); const total = ref(0)

const query = reactive({ keyword: '', status: undefined as boolean | undefined, page: 1, pageSize: 10 })
...
const mockUsers: User[] = Array.from({ length: 36 }, (_, i) => ({
  id: i + 1, username: `user_${i + 1}`, nickname: `用户${i + 1}`,
  email: `user${i + 1}@example.com`, role: i < 2 ? 'admin' : 'user',
  status: i % 5 !== 0,
...
function fetchData() {
  ...
  setTimeout(() => {
    let data = [...mockUsers]
    if (query.keyword) data = data.filter((u) => u.username.includes(query.keyword!) || u.email.includes(query.keyword!))
...
function openDialog(row?: User) {
  isEdit.value = !!row
  Object.assign(form, row ? { ...row } : { username: '', nickname: '', email: '', role: 'user', status: true })
...
function handleDelete(row: User) {
  ElMessageBox.confirm(t('deleteUserConfirm', { name: row.username }), t('confirm'), { type: 'warning' }).then(() => {
...
function toggleStatus(row: User) {
  ElMessage.success(row.status ? t('userEnabled', { name: row.username }) : t('userDisabled', { name: row.username }))
}
