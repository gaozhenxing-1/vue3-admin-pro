...
const mockRoles: Role[] = [
  {
    ...
    desc: '拥有系统全部权限',
    userCount: 2,
    permissions: ['仪表盘', '用户管理', '角色管理', '系统设置'],
    permissionIds: [1, 11, 2, 21, 22, 23, 24, 3, 31, 32, 33, 34, 4, 41, 42],
  },
  {
    id: 2,
    name: '编辑',
...
function openDialog(row?: Role) {
  ...
  Object.assign(
    ...
    row
      ...
      : { name: '', code: '', desc: '', userCount: 0, permissions: [], permissionIds: [] },
  )
...
function handleDelete(row: Role) {
  ElMessageBox.confirm(t('deleteRoleConfirm', { name: row.name }), t('confirm'), {
    type: 'warning',
  }).then(() => {
