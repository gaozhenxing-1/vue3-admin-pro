<template>
  <div class="dashboard">
    ...
    <el-alert type="info" :closable="false" style="margin-bottom:16px" show-icon>
      <template #title>
        ...
        <span style="margin-left:16px">
          <el-tag v-if="hasRole('admin')" size="small" type="danger" style="margin:2px">Admin Only</el-tag>
          <el-tag v-if="hasRole('editor')" size="small" type="warning" style="margin:2px">Editor+</el-tag>
          <el-tag v-if="hasRole('guest')" size="small" type="info" style="margin:2px">Guest+</el-tag>
          <span style="margin-left:8px;font-size:12px;color:#909399">← buttons visible by role</span>
        </span>
      </template>
    </el-alert>
    ...
    <el-row :gutter="16" style="margin-top:16px">
      ...
      <el-col :xs="24" :lg="8">
        <el-card shadow="hover" class="chart-card">
          <template #header><span>{{ t('sourceDistribution') }}</span></template>
    ...
    <el-row :gutter="16" style="margin-top:16px">
      <el-col :xs="24" :lg="14">
        <el-card shadow="hover" class="chart-card">
          <template #header><span>{{ t('orderTrend') }}</span></template>
          <v-chart :option="barOption" autoresize />
        </el-card>
      </el-col>
      <el-col :xs="24" :lg="10">
        <el-card shadow="hover" class="table-card">
          <template #header><span>{{ t('recentOrders') }}</span></template>
    ...
    <el-row :gutter="16" style="margin-top:16px">
      <el-col :xs="24" :lg="12">
        <el-card shadow="hover" class="table-card">
          <template #header><span>{{ t('popularProducts') }}</span></template>
      ...
      <el-col :xs="24" :lg="12">
        <el-card shadow="hover" class="table-card">
          <template #header><span>{{ t('latestNews') }}</span></template>
          <el-timeline>
            <el-timeline-item v-for="(item, i) in timeline" :key="i" :timestamp="item.time" placement="top" :color="item.color">
              {{ t(item.key) }}
            </el-timeline-item>
          </el-timeline>
        </el-card>
...
const weekData = { xAxis: ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], visits: [820,932,901,1290,1330,1020,980], orders: [120,145,132,188,201,160,155] }
const monthData = { xAxis: ['W1','W2','W3','W4'], visits: [3850,4200,5100,4680], orders: [550,620,780,690] }
...
const lineOption = reactive({
  ...
  yAxis: { type: 'value' },
  series: [
    { name: 'Visits', type: 'line', data: weekData.visits, smooth: true, itemStyle: { color: '#409eff' }, areaStyle: { color: 'rgba(64,158,255,0.1)' } },
    { name: 'Orders', type: 'line', data: weekData.orders, smooth: true, itemStyle: { color: '#67c23a' }, areaStyle: { color: 'rgba(103,194,58,0.1)' } },
  ],
})
...
const pieOption = {
  ...
  series: [{
    ...
    data: [
      { value: 1048, name: 'Search' }, { value: 735, name: 'Direct' },
      { value: 580, name: 'Social' }, { value: 484, name: 'Email' }, { value: 300, name: 'Ads' },
    ],
  }],
}
...
const barOption = {
  ...
  yAxis: { type: 'value' },
  series: [{ type: 'bar', data: [320,450,380,520,480,610], itemStyle: { color: '#409eff', borderRadius: [4,4,0,0] }, barWidth: '50%' }],
}

const recentOrders = [
  { orderNo: 'ORD-001', customer: '张三', amount: '¥299', statusType: 'success', statusKey: 'paid' },
  { orderNo: 'ORD-002', customer: '李四', amount: '¥580', statusType: 'success', statusKey: 'shipped' },
  { orderNo: 'ORD-003', customer: '王五', amount: '¥128', statusType: 'warning', statusKey: 'pending' },
  { orderNo: 'ORD-004', customer: '赵六', amount: '¥960', statusType: 'success', statusKey: 'paid' },
  { orderNo: 'ORD-005', customer: '孙七', amount: '¥450', statusType: 'danger', statusKey: 'refunded' },
]
...
.stat-value { font-size: 28px; font-weight: 700; color: var(--text-primary, #303133); line-height: 1.2; }
.stat-title { font-size: 14px; color: var(--text-secondary, #909399); margin-top: 2px; }
...
.chart-card :deep(.el-card__body) { height: calc(100% - 56px); }
.table-card { height: 390px; }
.table-card :deep(.el-card__body) { height: calc(100% - 56px); overflow-y: auto; }
</style>
